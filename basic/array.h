#pragma once

#include <stdio.h>

#include <cassert>
#include <initializer_list>
#include <type_traits>
#include <vector>

template <typename Type>
struct array {
    Type*  data  = nullptr;
    size_t count = 0;

    inline const Type& operator[](size_t i) const { return data[i]; }
    inline Type&       operator[](size_t i) { return data[i]; }
    inline const Type& back() const { return data[count - 1]; }
    inline Type&       back() { return data[count - 1]; }
    inline void        add(const Type& val) { data[count++] = val; }

    inline size_t size() const { return count; }
    inline void resize(size_t new_size) { count = new_size; }
    inline void push_back(const Type& val) { add(val); }

    array() {
        data  = nullptr;
        count = 0;
    }

    // array(const Type* p, int c) : data(p), count(c) {}
    array(Type* p, size_t c) : data(p), count(c) {}

    // Implicit view over a std::vector. Two overloads so a const-element view
    // (array<const T>) can be taken over a std::vector<T> as well.
    template <typename U = Type, std::enable_if_t<!std::is_const<U>::value, int> = 0>
    array(std::vector<U>& v) : data(v.data()), count(v.size()) {}
    template <typename U = Type, std::enable_if_t<std::is_const<U>::value, int> = 0>
    array(const std::vector<std::remove_const_t<U>>& v)
        : data(v.data()), count(v.size()) {}

    inline void insert(const Type& element, size_t index) {
        for (size_t i = count; i > index; i--) {
            data[i] = data[i - 1];
        }
        data[index] = element;
        count += 1;
    }

    inline void insert(const array<Type>& arr, size_t index) {
        for (size_t i = count; i > index; i--) {
            data[i] = data[i - arr.count];
        }
        for (size_t i = 0; i < arr.count; ++i) {
            data[index + i] = arr[i];
        }
        count += arr.count;
    }

    inline void operator+=(const array<Type>& arr) { insert(arr, count); }

    inline void remove(size_t index) {
        for (size_t i = index; i < count - 1; ++i) {
            data[i] = data[i + 1];
        }
        count -= 1;
    }

    void operator=(const std::initializer_list<Type>& list) {
        count    = list.size();
        size_t i = 0;
        for (auto& v : list) {
            data[i++] = v;
        }
    }

    array<Type> slice(size_t from, size_t to) {
        assert(from >= 0 and from <= count);
        assert(to >= 0 and to <= count);
        assert(from <= to);
        return {data + from, to - from};
    }

    const array<const Type> slice(size_t from, size_t to) const {
        assert(from >= 0 and from <= count);
        assert(to >= 0 and to <= count);
        assert(from <= to);
        return {data + from, to - from};
    }

    // iterators
    struct const_iterator {
        const Type* data;
        size_t      i;

        const_iterator& operator++() {
            i += 1;
            return *this;
        }
        bool operator!=(const const_iterator& other) const {
            return i != other.i;
        }
        const Type& operator*() const { return *(data + i); }
    };
    inline const_iterator begin() const { return const_iterator{data, 0}; }
    inline const_iterator end() const { return const_iterator{data, count}; }

    struct mutable_iterator {
        Type*  data;
        size_t i;

        mutable_iterator& operator++() {
            i += 1;
            return *this;
        }
        bool  operator!=(mutable_iterator& other) const { return i != other.i; }
        Type& operator*() const { return *(data + i); }
    };
    inline mutable_iterator begin() { return mutable_iterator{data, 0}; }
    inline mutable_iterator end() { return mutable_iterator{data, count}; }

    inline Type pop() {
        assert(count > 0);
        Type val = data[count - 1];
        count -= 1;
        return val;
    }
};

template <typename Type>
inline const Type* data(const array<Type>& arr) {
    return arr.data;
}

// fill array with a constant value
template <typename Type>
inline void fill(array<Type>& arr, const Type& val) {
    for (auto& x : arr) x = val;
}

// fill array with values returned by a function f: (int) -> Type
template <typename Type, typename Function>
inline void fill(array<Type>& arr, Function f) {
    for (size_t i = 0; i < arr.size(); ++i) arr[i] = f(i);
}

// copy content of array to another array
template <typename Type>
inline void copy_to(const array<Type>& from, array<Type>& to) {
    assert(from.count <= to.count);
    for (size_t i = 0; i < from.count; ++i) to[i] = from[i];
    to.count = from.count;
}

// copy content of array of array to another array of arrays
template <typename Type>
inline void copy_to(const array<array<Type>>& from, array<array<Type>>& to) {
    assert(from.count <= to.count);
    for (size_t i = 0; i < from.count; ++i) copy_to(from[i], to[i]);
    to.count = from.count;
}

// get index of fisrt occurrence of some value
template <typename Type>
inline size_t find(const array<Type>& arr, const Type& value) {
    for (size_t i = 0; i < arr.count; ++i)
        if (arr[i] == value) return i;
    return (size_t)-1;
}

template <typename Type, typename V>
inline size_t find(const Type& arr, const V& value) {
    for (size_t i = 0; i < arr.size(); ++i)
        if (arr[i] == value) return i;
    return (size_t)-1;
}

// check if array contains some value
template <typename Type>
inline bool contains(const array<Type>& arr, const Type& value) {
    return find(arr, value) != -1;
}

template <typename Type>
inline bool operator==(const array<Type>& a, const array<Type>& b) {
    if (a.count != b.count) return false;
    for (size_t i = 0; i < a.count; i++) {
        if (a[i] != b[i]) return false;
    }
    return true;
}

inline size_t argmin(const array<int>& arr) {
    assert(arr.count > 0);
    size_t min_index = 0;
    for (size_t i = 1; i < arr.count; ++i) {
        if (arr[i] < arr[min_index]) {
            min_index = i;
        }
    }
    return min_index;
}

inline size_t argmax(const array<int>& arr) {
    assert(arr.count > 0);
    size_t max_index = 0;
    for (size_t i = 1; i < arr.count; ++i) {
        if (arr[i] > arr[max_index]) {
            max_index = i;
        }
    }
    return max_index;
}

inline void shuffle(array<int>& arr) {
    for (size_t i = arr.count - 1; i > 0; --i) {
        int j   = rand() % (i + 1);
        int tmp = arr[i];
        arr[i]  = arr[j];
        arr[j]  = tmp;
    }
}
